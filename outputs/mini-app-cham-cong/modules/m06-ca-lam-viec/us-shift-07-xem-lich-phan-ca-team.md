...
Feature: US-SHIFT-07
  ...
  # --- AC1: Calendar Grid ---
  Scenario: AC1.1 — Calendar Grid
    Given Quản lý / Trưởng phòng (MANAGER, DEPT_HEAD) truy cập module
    When thực hiện "Calendar Grid"
    Then hiển thị kết quả chính xác. Dữ liệu phân quyền đúng RBAC.

  # --- AC2: Nhân sự summary per ngày ---
  Scenario: AC2.1 — Nhân sự summary per ngày
    Given Quản lý / Trưởng phòng (MANAGER, DEPT_HEAD) truy cập module
    When thực hiện "Nhân sự summary per ngày"
    Then hiển thị kết quả chính xác. Dữ liệu phân quyền đúng RBAC.

  # --- AC3: Gap Detection (Understaffing Alert) ---
  Scenario: AC3.1 — Gap Detection (Understaffing Alert)
    Given Quản lý / Trưởng phòng (MANAGER, DEPT_HEAD) truy cập module
    When thực hiện "Gap Detection (Understaffing Alert)"
    Then hiển thị kết quả chính xác. Dữ liệu phân quyền đúng RBAC.

  # --- AC4: Đề xuất đổi ca (Request) ---
  Scenario: AC4.1 — Đề xuất đổi ca (Request)
    Given Quản lý / Trưởng phòng (MANAGER, DEPT_HEAD) truy cập module
    When thực hiện "Đề xuất đổi ca (Request)"
    Then hiển thị kết quả chính xác. Dữ liệu phân quyền đúng RBAC.

  # --- Edge Case ---
  Scenario: Edge1 — NV chưa được gán ca
    Given NV mới chưa có lịch
    When hệ thống kiểm tra
    Then Ô trống (không màu). Tooltip: "Chưa phân ca." Đề xuất HR gán.

  # --- Edge Case ---
  Scenario: Edge2 — Team > 50 NV
    Given DEPT_HEAD có 80 NV
    When hệ thống kiểm tra
    Then Virtual scroll hàng NV. Lazy load. Search NV by name.

  # --- Edge Case ---
  Scenario: Edge3 — NV thuộc 2 ca cùng ngày
    Given Conflict chưa được xử lý
    When hệ thống kiểm tra
    Then Ô hiển thị 2 ca chồng + badge "⚠️ Xung đột" (Đỏ). Link HR: "Báo HR xử lý."
```
